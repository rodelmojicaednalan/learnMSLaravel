...
class LevelsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('backend.administrator.level.index', compact('page_title', 'page_description'));
    	...
    	switch ($section) {
				...
				if (Auth::check()) {
					...
					if(Auth::user()->hasRole('administrator')){
						...
						$level_obj = Level::where('id', $level_id)->first();
						...
						{
							$level_obj->subject = $request->subject;
							$level_obj->fee = $request->fee;
				...
				case 'add_new_child':
					...
					if (Auth::check()) {
						...
						if(Auth::user()->hasRole('administrator')){
							...
							if(empty($level_obj)) 
							{   
								$level_obj = Level::where('id', $request->parent_id)->first();
				...
				case 'load_levels':
					if (Auth::check()) {
						if(Auth::user()->hasRole('administrator')){
							// $data = Level::where('level_id', $request->id)->get();
							// return json_encode($data);
				...
				case 'load_parent':
					...
					if (Auth::check()) {
						if(Auth::user()->hasRole('administrator')){
						$data = Level::where('parent_id', "0")->get();
						return json_encode($data);
						}
					}
					 else
				...
				case 'load_child':
					if (Auth::check()) {
						if(Auth::user()->hasRole('administrator')){
						$data = Level::where('parent_id', $request->parent_id)->get();
						return json_encode($data);
						}
					}
					 else
