...
class PublicClassesController extends Controller
{
    ...
    {
        $page_title = 'Artbug Classes';
        $page_description = '';


		$users = User::role('teacher')->get();
		$subjects = Subject::get();
        return view('backend.administrator.public-classes.index', compact('page_title', 'page_description'))->with('subjects' , $subjects)->with('users' , $users);
    }
	...
	public function meeting()
    {
        ...
        return view('backend.administrator.public-classes.zoom', compact('page_title', 'page_description'));
    }
			...
			if (Auth::check()) {
				if(Auth::user()->hasRole('administrator'))
				{
					...
					else  
					{
						...
						$artclass_obj->teacher_id = $request->teacher;
    		...
    		case 'list':
				...
				if (Auth::check()) {
					if(Auth::user()->hasRole('administrator'))
					{
						...
						$query = ArtbugClass::with('schedule','user' , 'subject','level');
						...
						if(isset($search['query']['teacher']))
						{
							$query = $query->where('teacher_id', $search['query']['teacher']);
						}
			...
			case 'load_schedules':
				if (Auth::check()) {
					if(Auth::user()->hasRole('administrator')){
						...
						return view('backend.administrator.public-classes.ajax.schedule-modal')->with($data);
					}
				  }
				  else
				{
