        ...
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            ...
            <div class="card-toolbar">
                ...
                <span class="svg-icon svg-icon-md">
                    ...
                    <!--end::Svg Icon-->
                </span>Add New Parent Category</a>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
...
function loadSubjectLevels(id)
{
     $.ajax({
        type:'POST',
        url: 'categories/ajax/load_levels',        
        data:{
            id: id,
           _token: CSRF_TOKEN,
        },
...
$('#update-subject-level-form').on('submit',function(e){
                        ...
                        $.ajax({
                                type:'POST',
                                url: '/administrator/settings/categories/ajax/update_level',
                                dataType:"json",
                                data: $(this).serialize(),
                                success:function(data)
                                {
...
$(document).on('click','.action_user',function(e)
{
 ...
 if(action == "edit_child")
 {
    ...
    $("#txt_parentid").val(id);
    $('#txt_parentid').attr('subject_id', id);
    

    $('#child_txt').html("Edit Category");
    $('#childModal').modal("show");
    $('#btnUpdateChild').show();
    $('#btnAddChild').hide();
    ...
    $.ajax({
        type:'POST',
        url: 'categories/ajax/load_single_subject',
        dataType:"json",
        data:{
            id: id,
           _token: CSRF_TOKEN,
...
function get_child(parent_id,get_div) 
{
       ...
       $('#'+ get_div).html("");
        ...
        $.ajax({
        type:'POST',
        url: 'categories/ajax/load_child',
        dataType:"json",
        data:{
        parent_id:parent_id,   
        _token: CSRF_TOKEN,
        ...
        success:function(data)
        {
            ...
            for(var x=0; x < data.length; x++)
            {
                ...
                '</button>'+
                '</h5></div>'+
                '<div class="mt-3 col-md-4"><button class="btn btn-success action_user" data-action="edit_child" data-id="'+ data[x].id +'">Edit</button>&nbsp;<button class="btn btn-primary action_user" data-action="add_child"  data-id="'+ data[x].id +'">Add Child</button></div>'+
                '</div>'+
...
var subject = $('#child_level').val();
var parent_id = $('#txt_parentid').val();

$.ajax({
        type:'POST',
        url: 'categories/ajax/add_new_child',
        dataType:"json",
        data:{
            subject: subject,
            subject_id:0,
...
var subject = $('#child_level').val();
var parent_id = $('#txt_parentid').val();
var subject_id =  $('#txt_parentid').attr("subject_id");
...
$.ajax({
        type:'POST',
        url: 'categories/ajax/add_new_subject',
        dataType:"json",
        data:{
            subject: subject,
            subject_id:0,
...
$.ajax({
        type:'POST',
        url: 'categories/ajax/btnSaveLevel',
        dataType:"json",
        data:{
...
$.ajax({
        ...
        success:function(data)
        {
            ...
            html: err_txt,
                    ...
                    source: {
                        read: {
                            url: 'categories/ajax/subjects_list',
                            headers: {
        ...
        function parent_accordion()
        {
                $('#accordion').html("");
                        ...
                        $.ajax({
                        type:'POST',
                        url: 'categories/ajax/load_parent',
                        dataType:"json",
                        data:{
                        _token: CSRF_TOKEN,
                        },
