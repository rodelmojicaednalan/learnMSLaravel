			...
			<div class="col-md-6 col-xxl-6">
				<div class="pt-30 pt-md-25 pb-15 px-5 text-center">
                    ...
                    @foreach ($child as $item)
                        <span class="font-size-h4">{{ $item['name'] }}</span> -
                        <strong class="font-size-h4">{{ $item['fee'] }}</strong>
                        <br>
                    @endforeach
					<span class="font-size-h1 d-block d-block font-weight-boldest text-dark-75 py-2">Total {{ $total_fee }}</span>
					<h4 class="font-size-h6 d-block d-block font-weight-bold mb-7 text-dark-50">{{ $subject->subject->subject }}</h4>
                    ...
                    <form id="payment-form" data-checkout-type="parent"  data-url="{{ route('parent.payment.submit') }}">
                        <img src="{{ asset('media/stripe/Powered by Stripe - blurple.svg') }}" alt="Stripe">
                        <hr>
                        @foreach (request()->child as $item)
                            <input type="hidden" name="child_id[]" value="{{ $item }}">
                        @endforeach

                        <input type="hidden" name="class_id" value="{{ $subject->id }}">
                        <input type="hidden" name="subject_id" value="{{ $subject->subject_id }}">
                        <input type="hidden" name="subject_name" value="{{ $subject->subject->subject }}">
                        <input type="hidden" name="stripeToken" id="stripeToken">
                        <input type="hidden" name="return_url" value="{{ route('parent.classes', ['payment' => 'success']) }}">
                        @csrf
                        <div id="card-element">
